...
class Transaction(Base):
    ...
    id: Mapped[int] = mapped_column(Integer, primary_key=True, index=True)
    used_date: Mapped[Date] = mapped_column(Date, nullable=False)  # 使用日
    ...
    paid_date: Mapped[Date] = mapped_column(Date, nullable=False)  # 支払日（自動計算）
...
class Goods(Base):
    ...
    artist_id: Mapped[int] = mapped_column(Integer, ForeignKey('artists.id'), nullable=False)  # アーティストID
    title: Mapped[str] = mapped_column(String, nullable=False)  # タイトル
    release_date: Mapped[Date] = mapped_column(Date, nullable=False)  # リリース年月日
