...
def insert_sample_knowhows():
    """KNOWHOWテーブルにサンプルデータを挿入"""
    engine = create_engine(DATABASE_URL)

    with engine.connect() as connection:
        # 既存のサンプルデータを削除
        connection.execute(text("DELETE FROM knowhows WHERE major_category IN ('技術', '業務')"))
        ...
        for data in sample_data:
            connection.execute(text("""
                INSERT INTO knowhows (major_category, middle_category, title, keywords, content, display_order, is_deleted)
                VALUES (:major_category, :middle_category, :title, :keywords, :content, :display_order, FALSE)
            """), data)
